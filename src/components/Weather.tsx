...
export default function Weather({ weatherData }: any) {
  ...
  function formatDate(dt: number) { // Formats the date to: 'Day of the week, the month, day of the month, and time'.
    ...
    <div className='Body'>
        ...
        <div className='weather-body'>
          <div className='weather-body-left flex-box'>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ alignSelf: 'flex-end' }}>{weather.weather[0].main}</div>
            </div>
          </div>
          <div className='break-line flex-box'></div>
          <div className='weather-body-right flex-box'>
            <div>Feels like {weather.main.feels_like}&deg;C, {weather.weather[0].description}</div>
      ...
      <div className="forecast-container">
        ...
        <div className='forecast-items'>
          {dailyForecastArray.map((item: any) => <div key={item.dt} className='forecast-item'> 
            <div style={{ fontSize: '16px', fontWeight: '700' }}>{formatDate(item.dt)}</div>
            <img src={`http://openweathermap.org/img/wn/${item.weather[0].icon}@2x.png`} alt="weather icon" title={item.weather[0].description} />
            <div style={{ fontSize: '20px', fontWeight: '700' }}>{item.main.temp}&deg;C</div>
          </div>)}
        </div>
      </div>
    </div>
